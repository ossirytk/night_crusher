...
//process = ba.pulsen(1, 10000) : pm.djembe(60, 0.3, 0.4, 1): bit_crushed_drums:cubicnl<: freeverb
//process = _: bit_crushed_drums:cubicnl<: freeverb
process = ba.pulsen(1, 10000) : pm.djembe(60, 0.3, 0.4, 1): bit_crushed_drums:cubicnl<: freeverb
with {
    ...
    big_layout(x) = hgroup("The whole shebang [tooltip: Noice tooltip]", x);
    // TODO actually have two channels and move noise into the main thing
    bit_crushed_drums = _:_,nice_noise(ct_freq, q_amount, gain_vol) :> ba.bitcrusher(bit_slider)
    with{
        //n_col(x) = vgroup("Noise Bitcrush [tooltip: Noise tooltip]", x);
        n_col(x) = big_layout(vgroup("[0] Noise Bitcrush", x));
        ...
        bit_slider = n_col(hslider("[3] bit_depth",12,2,12,1));
    };

    cubicnl = ba.bypass1(bp, ef.cubicnl_nodc(drive:si.smoo,offset:si.smoo))
    with{
        //cnl_group(x)  = vgroup("cubicnl [tooltip: Reference:
        //    https://ccrma.stanford.edu/~jos/pasp/Cubic_Soft_Clipper.html]", x);
        cnl_group(x) = big_layout(vgroup("[1] Drive", x));
    ...
    with{
        ...
        parameters(x) = big_layout(hgroup("[2] Reverb", x));
